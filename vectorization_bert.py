...
# joblib.dump(vectors, 'vseapteki_bert.joblib')
# print('file with matrix for russian texts is saved')
# print('start build faiss index...')
# dim = 312
# index = faiss.IndexFlatL2(dim)
# index.add(vectors)
# faiss.write_index(index, "flat_rus_bert.index")
...
vectors = np.vstack(new_df['vector'].values)
print('finish vectorizing')
joblib.dump(vectors, 'theindependentpharmacy_bert.joblib')
print('file with matrix for english texts is saved')
print('start build faiss index...')
dim = 768
index = faiss.IndexFlatL2(dim)
index.add(vectors)
faiss.write_index(index, "flat_eng_bert.index")